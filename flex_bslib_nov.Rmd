...
.my_bg { background-color: rgb(235, 230, 230); color: white; border: 0px solid black; }
```
...
setwd("E:/ARC/Projects/A1C/github/nov/")
...
a1c_final <- a1c_final %>% filter(!is.na(age_group) & !is.na(glycemia))
sd <- SharedData$new(a1c_final, key = ~glycemia)

# Create a filter_checkbox widget
mth_filter <- filter_checkbox(
  id = "mth_filter",
  label = "Test Month",
  sharedData = sd,
  group = ~testmonth
)
# plot <- plot_ly(sd, x = ~catvvcount , y = ~reorder(Region,catvvcount) ) 
...
tcount <- n_distinct(a1c_final$oid)
    valueBox(
...
all_ct <- n_distinct(a1c_final$oid)
...
df <- a1c_final %>% 
      ...
      summarise(counts = n()) %>% mutate(percent = round(counts/sum(counts), digits = 2))

df %>% ggplot(., aes(x = glycemia, y = percent)) +
      geom_bar(
        aes(fill = raceeth),
        stat = "identity", position = position_dodge(0.8),
        width = 0.7
      ) + coord_flip() +
      facet_wrap(~c(age_group)) +
      scale_fill_brewer(direction = -1,palette = "YlOrRd") +
      geom_text(
        aes(label = paste0(100*percent,"%"), group = raceeth),
        position = position_dodge(0.8),
        vjust = 0.5, hjust = -0.05, size = 4,  angle=0) +
...
map_filter1 <- filter_slider("diabetic", "Diabetic%", a1c_dat, ~round(percent_diab/.75, digits = 1))
...
# load("./data/a1c_final.Rdata")
library(gtsummary)
a1c_final_distinct <- a1c_final%>% distinct(oid,glycemia,gender,raceeth,age,age_group,median_household_income)
...
t1 <- a1c_final |>
  tbl_summary(
    include = c(testmonth, A1c_value, glycemia, gender, race, age, age_group, ftd, BMI_cat, BMI),
    by = testmonth, missing = "no", 
    percent = "column",
    type = list(ftd ~ "dichotomous"),
    value = list(ftd ~ "Y"),
    label = list( A1c_value = "A1c%",
                  glycemia = "Glycemia",
                  race = "Race",
                  age = "Age",
                  age_group = "Age Group",
  ...
  bold_labels() %>%
  modify_caption("Donor characteristics by test months")%>%  
                              as_flex_table() %>%  
                              flextable::set_table_properties("autofit")
  
t1
  # as_gt() %>%
  # gt::tab_style_body(style = gt::cell_fill(color = "#cfe5ed"), values = c("A1c%","Gender","Age","BMI group"))
# as_hux_xlsx(t1, file = "E:/ARC/Projects/A1C/data/nov/t1.xlsx")
# download_file(
#   path = "E:/ARC/Projects/A1C/data/nov/t1.xlsx",
#   output_name = "Summary Table 1",
#   button_label = "donwload table (.xlsx)",
#   button_type = "primary",
#   has_icon = TRUE,
#   self_contained = TRUE
# )
...
```{r , warning=FALSE, message=FALSE}
t2 <- a1c_final |>
  select(testmonth, A1c_value, glycemia, gender, race, age, age_group, ftd, BMI_cat, BMI) |>
  tbl_strata(
    strata = testmonth,
    .tbl_fun =
      ~ .x |>
        tbl_summary(by = glycemia, missing = "no", percent = "row",
                    type = list(ftd ~ "dichotomous"),
                    value = list(ftd ~ "Y"),
                    label = list(A1c_value = "A1c%",
                                 glycemia = "Glycemia",
        ...
        # .header = "**{strata}**, N = {n}"
  as_flex_table() %>% flextable::set_table_properties("autofit") 

t2
# 
# as_hux_xlsx(t2, file = "E:/ARC/Projects/A1C/data/nov/t2.xlsx")
# download_file(
#   path = "E:/ARC/Projects/A1C/data/nov/t2.xlsx",
#   output_name = "Summary Table 2",
#   button_label = "donwload table (.xlsx)",
#   button_type = "primary",
#   has_icon = TRUE,
#   self_contained = TRUE
